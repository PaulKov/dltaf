...
def build_unit_rollup(items: Sequence[UnitRunStats]) -> UnitRollup:
    ...
    succeeded_units = sum(1 for item in items if str(item.status).lower() == "success")
    failed_units = len(items) - succeeded_units
    return UnitRollup(
...
class UnitProgressLogger:
    ...
    def record(self, item: UnitRunStats) -> None:
        ...
        if self.verbosity == "verbose":
            log_fn = self.logger.info if str(item.status).lower() == "success" else self.logger.warning
    ...
    def finish(self, items: Sequence[UnitRunStats]) -> None:
        ...
        self.logger.info(
            "Unit observability finished: total=%s ok=%s failed=%s rows=%s avg=%.3fs p50=%.3fs p95=%.3fs",
            rollup.total_units,
    ...
    def _emit_checkpoint(self) -> None:
        ...
        self.logger.info(
            "Unit progress: completed=%s/%s ok=%s failed=%s rows=%s elapsed=%.3fs",
            self._completed,
            self.total_units,
            self._succeeded,
            self._failed,
...
__all__ = ["UnitProgressLogger", "UnitRollup", "build_unit_rollup"]
